...
export default interface ISolutionRepository {
  createSolution(problemId: number, code: string, readMe: string, language: string): Promise<ISolutionDTO>;

  getSolutionList(problemId: number): Promise<ISolvedDTO[]>;
  getSolution(problemId: number): Promise<ISolutionDTO>;

  updateSolution(problemId: number, code: string, readMe: string, language: string): Promise<ISolutionDTO>;
