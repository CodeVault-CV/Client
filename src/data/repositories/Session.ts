...
import SessionDTO, { ISessionDTO, ISessionParams } from "../../core/dto/SessionDTO";
import ISessionRepository from "../../core/useCases/repository-interfaces/iSession";
...
export default class SessionRepository implements ISessionRepository {
  ...
  async updateSession(session: ISessionParams): Promise<ISessionDTO> {
    return await HTTP.put("/session", session).then(({ data }) => new SessionDTO(data));
  }
